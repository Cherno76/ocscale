...
- **Panel sizing**: window is 800 wide × ~660 default, user-non-resizable. Height auto-fits content via the `fit_panel` command (frontend measures the card after data/tab changes; Rust clamps 400–800 logical px and re-pins the macOS panel under the tray icon). Overview/Agents are two-column grids split by a `gridLine` divider; the Overview model/project/agent list is a fixed 168px scroll strip and the cost donut legend is capped at 6 rows (both with hidden scrollbars) so toggling the cost tab never resizes the panel; the session list is height-capped the same way.
