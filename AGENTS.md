...
- **External pricing file** (no hardcoded prices in `pricing.rs`): official DeepSeek rates and the small USD backstop live in a user-editable JSON — `data_dir/ocscale/pricing.json` (macOS: `~/Library/Application Support/ocscale/pricing.json`). On first run the app copies the bundled default from `core/snapshots/pricing.json`; price changes only require editing that file, never rebuilding. The JSON encodes the Beijing-time peak windows (09:00–12:00 & 14:00–18:00, `peak` = 2× `offpeak`), the weekend off-peak rule (weekends bill at the off-peak rate all day from `weekend_offpeak_from` = 2026-08-23), and CNY/1M rates for `deepseek-v4-flash` / `deepseek-v4-pro` / `deepseek-v4-flash-vision-exp` (cache-write bills at the cache-miss input rate). `pricing::cost` picks peak vs off-peak from the event timestamp (`ts_ms`). Stored as USD at the zh UI's 7.2 rate so `cost × 7.2` shows exact CNY; overrides the live LiteLLM entry, which prices cache writes at 0.
